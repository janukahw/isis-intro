...
- **Minimap ("YOU ARE HERE"):** the facility.html plan miniaturized as inline SVG in the HUD corner; per-chapter route in the `MAP` table (endpoints chained so the dot never jumps); dot color tracks identity (cyan H⁻/n⁰, pink p⁺); orbits the ring 3⅜×, rattles inside the TS1 block during moderation, and kinks 45° at the sample — the same Bragg deflection the flight path makes.
...
Compared every flight spline's top-down headings (signed turns from path tangents, sampled in-browser from the real `build()` output) against the minimap `MAP` routes. Confirmed matches: the straight chapters are straight on both layers; the synchrotron orbit is counterclockwise on both; the injection bend turns right on both (27° flown vs 120° drawn — schematic compression). Two real findings, both fixed:

- **Extraction kicked the wrong way.** The chapter-6 spline veered *left* out of the ring while the minimap corners *right* — and outward from a counterclockwise orbit (centre on your left) is rightward. Path, kicker frames, and labels mirrored to +x.
- **The Bragg kink never reached the map.** Chapter 11 flies a 45° deflection at the sample (the chapter's whole teaching point), but the dot sailed straight to a detector sitting on the undeflected axis — where a real diffractometer puts the beamstop. `MAP` now approaches the sample until k≈0.57 (the scatter event), then turns 45° left to a relocated detector POI, arriving as the detection flash fires (k≈0.98).
- Also: the chapter-9 rattle now eases its amplitude in and out, so the 8→9→10 endpoints chain exactly (the dot could previously hop ~5 px into the rattle).

Left as is, deliberately: the map's radial ring exit and its 90° corner at extraction (subway-map schematic — a tangential exit would re-lay-out the whole right half of the map), the 2.2-lap flythrough vs 3⅜-orbit dot (declared), and the map-only corners at linac→foil and injection (each 3D chapter is its own frame; the dissolve flies straight through them).

## 6. Gaze stability + the becoming prologue (2026-06-12)

- **180° view flips (user report: "source to linac, ~5–10 s in").** The smoothed lookTarget chases its goal at rate 3.2/s, so it trails a *moving* goal by speed/3.2 — but the default look point was a fixed 3% of path length ahead. Mid-chapter, where `glide'(t)` peaks the speed, the lag exceeded that lookahead in several chapters: the lookTarget fell behind the camera and `lookAt` flipped backward. Instrumenting `lookAt` measured it flying backward in the RFQ (5.5→8.7 s, a 176° snap — the report), extraction (3.9→8.7 s) and the beamline (3.9→8.7 s). Straight symmetric neon tunnels read almost the same both ways, which is how it survived visual checks; chapters with mid-flight scripted gazes (foil, muon, target, detect) were masked. Fix: the default lookahead now scales with live path speed (1.6× the lag), and the RFQ vane glance releases at t 0.38 (was 0.42) while the vane is still ~3.5 units ahead — the late handoff used to drop the lookTarget right beside the advancing camera.
- **The becoming prologue:** chapter 1 now opens *outside* the ion — a pink proton glow with two cyan electrons orbiting it — and dives in until you merge with it (flash at t≈0.31), then the original chamber look-around and extraction play out. Duration 14 → 16 s; captions unchanged ("you are one proton, holding two borrowed electrons" now plays over the thing it describes).
