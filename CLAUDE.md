...
- **Vanilla HTML/CSS/JS, no build step, no dependencies** (only external: Google Fonts, which degrades to system fonts offline). Must work via `file://` AND a static server. One pinned, vendored exception: `vendor/three.min.js` (Three.js r149 UMD, MIT — the last non-ESM build, because ES modules can't load over `file://`), loaded **only** by `bethebeam.html`. Don't add others, and don't upgrade it past r159 (UMD builds were removed).
- Therefore **no `fetch()`, no ES modules.** Every JS file is a plain `<script defer>` IIFE exposing one global: `MEPlot` (canvas plotter), `MESynth` (all synthetic physics), `MEQuiz` (quiz engine), plus per-page scripts (bethebeam adds `BTBAudio` and `BTBScenes`).
...
- Pages: `index.html` (animated story) · `probes.html` (why neutrons/muons) · `facility.html` (how ISIS works) · `ts1.html` / `ts2.html` (deep dives: TS1/TS2 target-hall particle simulations — subnav/card-linked from facility.html, not in the main nav) · `experiment.html` (experiment lifecycle) · `data.html` (TOF, events, reduction playground) · `techniques.html` (9 techniques) · `how.html` (meta: how the site was built — footer-linked only, served as `/how` on Pages). Each topic page ends with a quiz and cross-link cards.
- The Facility section is tabbed via the `.subnav` link strip (no JS): `facility.html` (Overview) · `ts1.html` (Target Station 1 — the live hall particle simulation) · `ts2.html` (Target Station 2 — cold-neutron station) · `bethebeam.html` (BE THE BEAM — first-person WebGL ride, ion source → detector event). These tabs are not in the main nav; they're reached through the subnav and facility cross-links.
- `bethebeam.html` specifics: the ride is one continuous flight — no stops; chapters dissolve into each other (render-target crossfade in `js/bethebeam.js`, with the outgoing camera still moving); in-ride text is short HUD captions in `js/bethebeam-scenes.js`, which must stay consistent with the in-page storyboard HTML — the storyboard is the canonical reading layer. The HUD minimap (inline SVG in the overlay; per-chapter route in the `MAP` table in `js/bethebeam.js`) mirrors the facility.html plan — keep its endpoints chained so the dot never jumps across a dissolve. HUD speeds derive from energies via `MESynth.protonBeta`/`neutronBeta`; the soundtrack is synthesized live with Web Audio (`BTBAudio`) — user-gesture-started only, always with a visible mute, no audio files in the repo. No WebGL or reduced motion ⇒ the storyboard is the ride.
